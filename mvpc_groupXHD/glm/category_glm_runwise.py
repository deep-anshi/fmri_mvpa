...
# '/home/nastase/attention/raw_bids'
base_dir = '/dartfs/rc/lab/D/DBIC/DBIC/datasets.datalad.org/labs/haxby/attention'
...
glm_dir = join('/dartfs-hpc/scratch/psyc164/groupXHD/analysis', 'sub-' + participant, 'func')
...
for run in [1, 2, 3, 4, 5]:
    ...
    for motion_reg in ['X', 'Y', 'Z', 'RotX', 'RotY', 'RotZ']:
        ...
        assert len(motion_demean) == len(
            motion_deriv) == len(ortvec[motion_reg])
        ortvec[motion_reg +
               '_demean'] = ['{:.9f}'.format(m) for m in motion_demean]
        ortvec[motion_reg +
               '_deriv'] = ['{:.9f}'.format(m) for m in motion_deriv]
